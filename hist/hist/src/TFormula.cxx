...
bool TFormulaParamOrder::operator() (const TString& a, const TString& b) const {
   ...
   // note that any parameter name that starts with a lowercase 'p' goes to the
...
void TFormula::HandleParametrizedFunctions(TString &formula)
{
   ...
   for(map<pair<TString,Int_t>,pair<TString,TString> >::iterator it = functions.begin(); it != functions.end(); ++it)
   {
	 ...
	 if(defaultCounter)
         ...
         {
            // Check whether this is just a number in parentheses. If not, leave it to `HandleUserFunctions` to be parsed
...
void TFormula::HandleParamRanges(TString &formula) {
   ...
   while ((matchIdx = rangePattern.Index(formula, len, matchIdx)) != -1) {
      ...
      if (endCnt <= startCnt)
	 Error("HandleParamRanges",
	       "End parameter (%d) smaller than start parameter (%d) in parameter range",
	       endCnt, startCnt);
...
///   TODO: a better name would be HandleFunctionArguments
	    ...
	    if (nArguments == ndim + npar) {
		  ...
		  if (argNr < ndim) { // variable
			...
			argSubstitutions[defaultVariableNames[argNr]] = newName;
		     ...
		     TString oldName = (f) ?
			TString::Format("[%s]", f->GetParName(parNr)) :
			TString::Format("[%d]", parNr);
		     argSubstitutions[oldName] = newName;
	       ...
	       canReplace = true;
               ...
               // Try to assume variables are implicit
   ...
   for(Int_t i = 0 ; i < formula.Length(); ++i )
   {
      ...
      if(isalpha(formula[i]) && !IsOperator(formula[i]))  // not really needed to check if operator (if isalpha is not an operator)
      {
         ...
         else
         {
            ...
            if (f) {
	       // Replacing user formula the old way (as opposed to 'HandleUserFunctions')
...
//
   ...
   else
   {
      ...
      if (ret.second) {
         ...
         // for (auto & p : fParams)
         //    std::cout << "Parameter " << p.first << " position " << p.second << " value " << fClingParameters[p.second] << std::endl;
...
void TFormula::Print(Option_t *option) const
{
   ...
   {
      // we can skip this
      Info("Print","Not all parameters are setted."); // TODO comment out again
